...
| Campo | Conteúdo |
|-------|----------|
| Descrição | Policies `USING (true)` + SELECT anon |
| Consequência | Vazamento de perfis |
| Prioridade | P0 |
| Módulos | Supabase RLS |
| Restrição | Não criar policies abertas semelhantes |
| Correção futura | Endurecer policies |
| Status | Aberta |
