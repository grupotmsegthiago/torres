...
| Campo | Conteúdo |
|-------|----------|
| Descrição | Coluna texto legada; valores **já limpos** (36/36 NULL). Código/tipos **desacoplados** (PR4A). Artefatos de DROP versionados (PR4B) — **não aplicados**. RLS + `toSafeUser` + `sanitizeUserWrite` ativos. |
