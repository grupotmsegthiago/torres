...
export async function resolveHorasExtrasNoturnas(opts: {
  ...
  // Fallback: batidas Control iD (mesma lógica da Folha: hoursWorked − limite mensal).
  // Só em janela ~mensal — evita jogar HE do mês inteiro num filtro de semana/dia.
  if (allowBatidas) {
    try {
...
export function composeCustoEmpresaDetalhado(input: {
  ...
  const custoRealizado = r2(
    folha.totalBruto + extras + diariasN + inssPatronal + seguroVida,
  );
