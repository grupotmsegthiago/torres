...
## Pedágio do boletim ≠ pedágio em mission_costs nem sempre é bug
- **billing.despesas_pedagio < real (split de mission_costs)** = boletim calculado cedo/recalc derrubou pedágio → corrigir só se NÃO faturada (sem invoice_id): `despesas_pedagio = real`, `fat_total += dif` (mantém total = soma das colunas).
- **billing.despesas_pedagio > real** = assinatura de **ajuste manual do dono** (ex.: zerou/colocou valor redondo de propósito). NUNCA auto-sobrescrever — confirmar antes.
- Faturadas (invoice_id setado): NÃO mexer no registro mesmo se subfaturado — a fatura já saiu no valor antigo; diferença é decisão de negócio (cobrar à parte), não correção silenciosa.
- **Why:** dono confirmou que edita pedágio à mão (caso TOR-0262: zerou 292,11 de propósito); sincronizar cegamente desfaz o trabalho dele.
