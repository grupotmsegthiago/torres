...
- **Aviso WhatsApp persiste status p/ o RH não ficar no escuro.** O dispatcher (`notifyDocsBackground(targets, title, isReminder, persist)`) recebe `{docId, emp}` e chama `persist(docId, status)`; status ∈ `enviado|sem_telefone|bloqueado|falha` (gravado em `whatsapp_notify_status`/`whatsapp_notify_at`). **Why:** antes o boolean era descartado e a falta de telefone OU o gate FAIL-CLOSED de número oficial (chip da Central errado) deixava o vigilante sem aviso em SILÊNCIO. **How:** `notifyEmployeeDoc` checa telefone → `assertExpectedNumber` (bloqueado) → envia; o dashboard RH (`documentos-rh`) mostra badge por linha + banner global vermelho quando há `bloqueado`. Best-effort: persistência nunca quebra a emissão.
