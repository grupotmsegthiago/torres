...
**How to apply:** o status confiável vem do próprio `/api/operational-grid` (campo `billingStatus`, lido direto de `escort_billings`). Qualquer tela que precise saber se a OS já tem boletim congelado deve usar essa fonte (ou consultar escort_billings), nunca inferir por ausência no byMission. O total do modal soma as OSs listadas — não reutiliza `totals.fatAberto` (números do card do Balanço ficam intocados).
