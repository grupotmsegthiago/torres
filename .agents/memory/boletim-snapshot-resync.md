...
**Regra:** o snapshot "foto única" do boletim de medição (boletim_approvals.billing_snapshot) só vale enquanto o status das OSs não muda. Quando uma OS vira **recusada** ou **cancelada** depois do envio, todo boletim **PENDENTE** que a contém precisa ser re-sincronizado: recusada SAI do boletim (ordem do dono 20/07/2026, revisada no mesmo dia — antes entrava zerada; boletim leva só APROVADAS + CANCELADAS), cancelada é recongelada com os valores atuais do billing (tabela 100 km). Boletim APROVADO nunca é tocado. Excel tem coluna STATUS e a coluna Nº prefere o os_number ATUAL da OS (o do billing pode estar stale "OS-nnn").
...
- Envio (`enviar-aprovacao`) valida elegibilidade no BACKEND via `billingElegivelParaBoletim` (boletim-totals): recusada FORA, FATURADO/FATURADA/PAGO fora; canceladas entram. (`billingTotalForBoletim` segue retornando R$0 p/ recusada como rede de segurança, §8.1.) Nunca confiar na seleção do front.
