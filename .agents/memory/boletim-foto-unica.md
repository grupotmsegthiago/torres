...
- **Reenvio = MESMO boletim, nunca recriar** (`POST /api/boletim/reenviar/:id`): recarrega o approval, sobrescreve os componentes do billing pelo `billing_snapshot` e reenvia com o mesmo token/total. Recriar via `/enviar-aprovacao` com force quebra quando as OS já viraram fatura (billingElegivelParaBoletim filtra FATURADAS ⇒ e-mail sai só com as sobras). O botão "Reenviar Boletim" do front chama o reenvio quando há activeApproval. Rota usa o `requireAdminRole` REAL de `server/auth.ts` (o local do arquivo só checa req.user).
