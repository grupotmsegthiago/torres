...
## Exceção: a APROVAÇÃO agora usa a tabela ATUAL da OS (fix jul/2026)
O `POST /api/escort/billings/:id/revisar` (ação APROVADA) recalculava com `billing.contract_id` (congelado) → se o dono trocasse a tabela da OS depois do billing criado, aprovar **revertia** os valores pra tabela antiga (caso real: OS reassinada 200km→100km, aprovar voltava pra 200km e zerava o KM excedente). Fix: resolve o contrato por `so.escort_contract_id` com fallback pro `billing.contract_id`, e ressincroniza `updateData.contract_id` + registra o de/para no audit (`APROVAR_MISSAO`). **Efeito colateral consciente:** qualquer OS `A_VERIFICAR` cujo `escort_contract_id` foi trocado passa a recalcular pela tabela nova ao aprovar — é o comportamento desejado, mas troca acidental de tabela muda o valor silenciosamente (mitigado pelo audit). §8: aprovação continua respeitando recusada=R$0 (§8.1), timestamps reais de HE (§8.5) e é UPDATE por id (§8.6).
