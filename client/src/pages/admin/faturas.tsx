...
export default function FaturasPage() {
  ...
  const [showTrace, setShowTrace] = useState<Invoice | null>(null);
                        ...
                        >
                          ...
                          <TableCell>
                            <div className="flex items-center justify-center gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
                              ...
                              <button
                                onClick={(e) => { e.stopPropagation(); setShowTrace(inv); }}
                                className="p-1.5 rounded-md hover:bg-amber-50 text-neutral-400 hover:text-amber-600 transition-colors"
                                title="Rastreio completo (rota do dinheiro)"
                                data-testid={`button-trace-${inv.id}`}
                              >
...
function CreateInvoiceDialog({ clients, asaasConnected, onClose }: { clients: any[]; asaasConnected: boolean; onClose: () => void }) {
  ...
  const [form, setForm] = useState({
    ...
    notes: "",
    sendToAsaas: true,
    // Banco Inter desativado — novas cobranças apenas via Asaas.
    gateway: "asaas",
  });
  ...
  const createMutation = useMutation({
    mutationFn: async () => {
      if (form.gateway === "inter") {
        throw new Error("Integração Banco Inter desativada. Use Asaas.");
      }
      try { localStorage.setItem("invoice_gateway", "asaas"); } catch (_) {}
