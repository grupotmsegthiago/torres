...
export default function RelatorioAbastecimentoPage() {
  ...
  // Eficiência REAL no MESMO formato da TicketLog:
  //  • Litros do período  = soma dos litros das abastecidas no período.
  //  • Km rodados período = pra cada abastecida no período, gap de
  //    hodômetro contra a abastecida imediatamente anterior do mesmo
  //    veículo (mesmo que essa anterior seja FORA do período).
  ...
  const eficienciaGeral = useMemo(() => {
    ...
    const tl = computeTicketlogStats(fuelings, inPeriod);
    return {
      kmPeriodo: tl.totalKmRodados,
      litrosPeriodo: tl.totalLiters,
      mediaKmL: tl.avgKmPerLiter,
    };
