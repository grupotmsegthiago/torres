...
// Cálculo dos KPIs extraído para client/src/lib/balanco-calc.ts — COMPARTILHADO
// com o Gestor de Dados Financeiro (os dois painéis mostram o MESMO número).
import {
  FROZEN_BILLING_STATUSES, isOsAberta,
  buildMissoesPeriodo, buildTotaisBalanco, buildEficiencia,
} from "@/lib/balanco-calc";
...
export default function BalancoGerencialPage() {
  ...
  const filtered = useMemo(() => buildMissoesPeriodo(data, gridData, range), [data, gridData, range]);
  ...
  const totals = useMemo(
    () => buildTotaisBalanco(filtered, provisaoRH, Number(fixedCostsSummary?.monthly || 0), costDays),
    [filtered, provisaoRH, fixedCostsSummary, costDays],
  );

  const eficiencia = useMemo(() => buildEficiencia(data, allVehicles, range), [data, allVehicles, range]);
