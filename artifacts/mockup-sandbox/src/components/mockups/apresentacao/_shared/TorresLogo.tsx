export function TorresLogoSVG({ size = 80, color = "#1a1a1a" }: { size?: number; color?: string }) {
  return (
    <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 300 380" width={size} height={size * (380 / 300)} fill="none">
      <g fill={color}>
...
export function TorresFullLogo({ size = 100, color = "#1a1a1a" }: { size?: number; color?: string }) {
  return (
    <div style={{ display: "flex", flexDirection: "column", alignItems: "center", gap: size * 0.08 }}>
      <TorresLogoSVG size={size} color={color} />
      <div style={{ display: "flex", flexDirection: "column", alignItems: "center", color }}>
        <span style={{ fontFamily: "'Montserrat', sans-serif", fontWeight: 900, fontSize: size * 0.28, letterSpacing: "0.06em", lineHeight: 1.1 }}>
          TORRES
        </span>
        <span style={{ fontFamily: "'Montserrat', sans-serif", fontWeight: 500, fontSize: size * 0.1, letterSpacing: "0.12em", lineHeight: 1.4 }}>
