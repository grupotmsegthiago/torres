...
var init_supabase = __esm({
  "server/supabase.ts"() {
    ...
    if (!isServerSupabaseConfigured()) {
      console.warn(
        "[supabase] SUPABASE_URL / SUPABASE_SERVICE_ROLE_KEY ausentes \u2014 APIs que usam o admin falhar\xE3o at\xE9 configurar."
      );
    }
    MAX_RETRIES = 2;
    BASE_DELAY_MS = 400;
    ...
    supabaseAdmin = createClient(supabaseUrl, supabaseServiceKey, sharedOpts);
    supabaseAnon = createClient(supabaseUrl, supabaseAnonKey, sharedOpts);
  }
});
...
__export(payroll_exports, {
  ...
  INSS_2025: () => INSS_2025,
  INSS_FAIXAS_SALARIO: () => INSS_FAIXAS_SALARIO,
  INSS_FLAT_PCT_PADRAO: () => INSS_FLAT_PCT_PADRAO,
  INSS_PROVISAO_FERIAS_13: () => INSS_PROVISAO_FERIAS_13,
  IRRF_2024: () => IRRF_2024,
  IRRF_FLAT_PCT_PADRAO: () => IRRF_FLAT_PCT_PADRAO,
  PERICULOSIDADE_PADRAO: () => PERICULOSIDADE_PADRAO,
  VALOR_HORA_EXTRA_FIXO: () => VALOR_HORA_EXTRA_FIXO,
  VALOR_HORA_NOTURNA_FIXO: () => VALOR_HORA_NOTURNA_FIXO,
  calcularFolha: () => calcularFolha,
  calcularINSS: () => calcularINSS,
  calcularIRRF: () => calcularIRRF,
  calcularInssPorFaixaSalario: () => calcularInssPorFaixaSalario,
  hhmmToDecimal: () => hhmmToDecimal,
  inssAliquotaPorFaixa: () => inssAliquotaPorFaixa,
  r2: () => r2
});
function inssAliquotaPorFaixa(salarioMensal) {
  const s = Number(salarioMensal) || 0;
  for (const f of INSS_FAIXAS_SALARIO) if (s <= f.ate) return f.pct;
  return 14;
}
function calcularInssPorFaixaSalario(salarioMensal) {
...
function calcularFolha(input) {
  ...
  const inss = isClt ? inssModo === "flat" ? calcularInssPorFaixaSalario(salarioProporcional + periculosidade) : calcularINSS(baseTributavel) : 0;
...
var INSS_2025, IRRF_2024, FGTS_ALIQUOTA, PERICULOSIDADE_PADRAO, VALOR_HORA_EXTRA_FIXO, VALOR_HORA_NOTURNA_FIXO, INSS_FLAT_PCT_PADRAO, INSS_FAIXAS_SALARIO, IRRF_FLAT_PCT_PADRAO, INSS_PROVISAO_FERIAS_13;
var init_payroll = __esm({
  "server/lib/payroll.ts"() {
    ...
    VALOR_HORA_EXTRA_FIXO = 16;
    VALOR_HORA_NOTURNA_FIXO = 16.5;
    INSS_FLAT_PCT_PADRAO = 9;
    INSS_FAIXAS_SALARIO = [
      { ate: 1621, pct: 7.5 },
      { ate: 2902.84, pct: 9 },
      { ate: 4354.27, pct: 12 },
...
async function buildFolhaStats(employeeId, monthYear, opts = {}) {
  ...
  const horasNoturnasRelogio = dias.reduce((s, d) => s + (Number(d.noturnoMin) || 0), 0) / 60;
  ...
  if (faixas) {
    try {
      ...
      const { data: absRows } = await supabaseAdmin.from("employee_absences").select("id, type, start_date, end_date, status").eq("employee_id", employeeId).eq("status", "aprovado").gte("start_date", `${absFrom}T00:00:00`).lte("start_date", `${absTo}T23:59:59`);
  ...
  const holidaySet = await loadHolidaySet2(civilFrom, civilTo);
  ...
  const civilStartBrt = /* @__PURE__ */ new Date(`${civilFrom}T00:00:00-03:00`);
  const civilEndBrt = /* @__PURE__ */ new Date(`${civilTo}T23:59:59-03:00`);
  ...
  const diasUteisTotal = countBusinessDays2(civilFrom, civilTo, holidaySet);
  const diasUteis = isCivilCorrente ? countBusinessDays2(civilFrom, cutoffCivilIso, holidaySet) : isCivilFuturo ? 0 : diasUteisTotal;
  ...
  const { VALOR_HORA_EXTRA_FIXO: VALOR_HORA_EXTRA_FIXO2, VALOR_HORA_NOTURNA_FIXO: VALOR_HORA_NOTURNA_FIXO2 } = await Promise.resolve().then(() => (init_payroll(), payroll_exports));
  const valorHoraExtra = VALOR_HORA_EXTRA_FIXO2;
  const valorHoraNoturna = VALOR_HORA_NOTURNA_FIXO2;
  ...
  let diarias = 0;
  try {
    const { data: diariaRows } = await supabaseAdmin.from("operational_payments").select("amount").eq("employee_id", employeeId).eq("type", "diaria").gte("payment_date", civilFrom).lte("payment_date", cutoffCivilIso);
  ...
  const { calcularInssPorFaixaSalario: calcularInssPorFaixaSalario2 } = await Promise.resolve().then(() => (init_payroll(), payroll_exports));
  const salarioMensalComPeric = baseSalary * fatorPericVH;
  const inssFuncionario = isClt ? calcularInssPorFaixaSalario2(salarioMensalComPeric) : 0;
  const irrfFuncionario = 0;
...
async function buildReconciliation(opts = {}) {
  ...
  const { data: maps } = await supabaseAdmin.from("control_id_users_map").select("id, employee_id, device_id, control_id_user_id, control_id_user_name, ativo, created_at").in("employee_id", empIds.length ? empIds : [-1]).eq("ativo", true);
  const mapsByEmp = /* @__PURE__ */ new Map();
  for (const m of maps || []) {
  ...
  const primaryMap = (arr) => {
    if (!arr || !arr.length) return null;
    return [...arr].sort(
      (a, b) => String(b.created_at || "").localeCompare(String(a.created_at || "")) || Number(b.id) - Number(a.id)
    )[0];
  };
  ...
  for (const emp of employees) {
    ...
    if (empMaps.length > 1) warnings.push(`Pessoa duplicada no RHID: ${empMaps.length} v\xEDnculos ativos (uids ${allRhidUserIds.join(", ")}) \u2014 batidas somadas de todos; unificar cadastro no portal Control iD`);
...
async function sendVencimentosDoDiaEmail2(opts) {
...
// server/lib/invoice-allocation.ts
function ratearRecebimento(itensIn, valorRecebido) {
  const itens = itensIn.filter((i) => Number.isFinite(i.valorItem) && i.valorItem >= 0);
  const totalItens = r22(itens.reduce((s, i) => s + i.valorItem, 0));
  const recebido = Math.max(0, r22(Number(valorRecebido) || 0));
  if (recebido >= totalItens - TOLERANCIA_CENTAVOS && totalItens > 0) {
    return {
      itens: itens.map((i) => ({ billingId: i.billingId, valorItem: r22(i.valorItem), valorAlocado: r22(i.valorItem), quitado: true, origem: "integral" })),
      totalItens,
      totalAlocado: totalItens,
      integral: true,
  ...
  for (const i of itens) {
    ...
    if (Number.isFinite(manual) && manual > 0) {
      const aloc = r22(Math.min(manual, i.valorItem, restante));
      restante = r22(restante - aloc);
      out.push({ billingId: i.billingId, valorItem: r22(i.valorItem), valorAlocado: aloc, quitado: aloc >= i.valorItem - TOLERANCIA_CENTAVOS, origem: "manual" });
    } else {
      semManual.push(i);
    }
  }
  ...
  if (restante > 0 && identificados.length === 1) {
    for (const i of semManual) {
      ...
      out.push({ billingId: i.billingId, valorItem: r22(i.valorItem), valorAlocado: isAlvo ? r22(restante) : 0, quitado: isAlvo, origem: "identificado" });
    }
    restante = 0;
  } else {
    ...
    semManual.forEach((i, idx) => {
      ...
      if (idx === semManual.length - 1) {
        aloc = r22(restante - acumulado);
      } else {
        aloc = base > 0 ? r22(i.valorItem / base * restante) : 0;
      }
      aloc = Math.max(0, Math.min(aloc, r22(i.valorItem)));
      acumulado = r22(acumulado + aloc);
      out.push({ billingId: i.billingId, valorItem: r22(i.valorItem), valorAlocado: aloc, quitado: aloc >= i.valorItem - TOLERANCIA_CENTAVOS, origem: "proporcional" });
    });
    restante = 0;
  }
  const totalAlocado = r22(out.reduce((s, i) => s + i.valorAlocado, 0));
  return {
    itens: out,
    totalItens,
    totalAlocado,
...
var TOLERANCIA_CENTAVOS, r22;
var init_invoice_allocation = __esm({
  "server/lib/invoice-allocation.ts"() {
    "use strict";
...
async function ensureInvoiceItems(invoiceId) {
  const { data: existing } = await supabaseAdmin.from("invoice_billing_items").select("*").eq("invoice_id", invoiceId);
  const byBilling = new Map((existing || []).map((it) => [String(it.billing_id), it]));
  const { data: billings } = await supabaseAdmin.from("escort_billings").select("id, service_order_id, fat_total, status").eq("invoice_id", invoiceId);
  if (!billings || billings.length === 0) return existing || [];
  const total = billings.reduce((s, b) => s + r23(b.fat_total), 0);
  const faltantes = billings.filter((b) => !byBilling.has(String(b.id)));
  if (faltantes.length > 0) {
    const rows = faltantes.map((b) => ({
      invoice_id: invoiceId,
      billing_id: b.id,
      service_order_id: b.service_order_id,
      valor_item: r23(b.fat_total),
      participacao_pct: total > 0 ? r23(r23(b.fat_total) / total * 100) : 0,
      valor_alocado: 0,
      alocacao_origem: null
    }));
    const { error } = await supabaseAdmin.from("invoice_billing_items").upsert(rows, { onConflict: "invoice_id,billing_id", ignoreDuplicates: true });
    if (error) {
      console.error("[invoice-payment] ensureInvoiceItems falhou:", error.message);
      return existing || [];
    }
  }
  const { data: fresh } = await supabaseAdmin.from("invoice_billing_items").select("*").eq("invoice_id", invoiceId);
  return fresh || [];
}
async function applyPaymentToInvoice(opts) {
  ...
  const { data: acumuladoRaw, error: rpcErr } = await supabaseAdmin.rpc("os_fin_registrar_recebimento", {
    p_event_key: eventKey,
    p_invoice_id: invoiceId,
    p_valor: r23(opts.valorRecebido),
    p_origem: origem
  });
  if (rpcErr) throw new Error(`os_fin_registrar_recebimento: ${rpcErr.message}`);
  if (acumuladoRaw == null) {
    console.log(`[invoice-payment] Fatura #${invoiceId}: evento ${eventKey} j\xE1 processado \u2014 ignorando (idempot\xEAncia)`);
    return {
      integral: false,
  ...
  const acumulado = r23(Number(acumuladoRaw));
  const entrada = items.map((it) => ({
    billingId: String(it.billing_id),
    valorItem: r23(it.valor_item),
    valorAlocadoManual: it.alocacao_origem === "manual" ? r23(it.valor_alocado) : null
  }));
  const rateio = ratearRecebimento(entrada, acumulado);
  const agora = (/* @__PURE__ */ new Date()).toISOString();
  const statusAntes = /* @__PURE__ */ new Map();
  {
    const { data: bs } = await supabaseAdmin.from("escort_billings").select("id, status").in("id", rateio.itens.map((i) => i.billingId));
  ...
  for (const it of rateio.itens) {
    ...
    if (it.quitado && antes !== "PAGO") {
      await supabaseAdmin.from("escort_billings").update({ status: "PAGO", pago_em: agora }).eq("id", it.billingId);
    } else if (!it.quitado && antes === "PAGO") {
      await supabaseAdmin.from("escort_billings").update({ status: "FATURADO", pago_em: null }).eq("id", it.billingId);
    }
  }
  ...
  const itemByBilling = new Map(items.map((it) => [String(it.billing_id), it]));
...
async function revertInvoicePayment(opts) {
  const { invoiceId } = opts;
  const agora = (/* @__PURE__ */ new Date()).toISOString();
  const { data: billings } = await supabaseAdmin.from("escort_billings").select("id, service_order_id, status").eq("invoice_id", invoiceId);
  await supabaseAdmin.from("invoice_billing_items").update({ valor_alocado: 0, alocacao_origem: null, alocado_em: agora }).eq("invoice_id", invoiceId);
  await supabaseAdmin.from("invoices").update({ valor_recebido: 0, updated_at: agora }).eq("id", invoiceId);
  const pagos = (billings || []).filter((b) => b.status === "PAGO");
  if (pagos.length > 0) {
    await supabaseAdmin.from("escort_billings").update({ status: "FATURADO", pago_em: null }).in("id", pagos.map((b) => b.id));
  }
  await audit((billings || []).map((b) => ({
    billing_id: b.id,
    service_order_id: b.service_order_id,
    invoice_id: invoiceId,
  ...
  console.log(`[invoice-payment] Fatura #${invoiceId}: revers\xE3o por ${opts.motivo} via ${opts.origem} \u2014 ${pagos.length} OS revertidas de PAGO`);
  return pagos.length;
}
var r23;
var init_invoice_payment = __esm({
  "server/lib/invoice-payment.ts"() {
    "use strict";
    init_supabase();
    init_invoice_allocation();
    r23 = (v) => Math.round((Number(v) || 0) * 100) / 100;
  }
});

// shared/constants/mission-status.ts
function contaComoAprovadaParaFatura(osStatus, billingStatus, osMissionStatus) {
  if (osStatus === "recusada" || osStatus === "cancelada") return false;
  if (billingStatus === "APROVADA") return true;
  const isOsConcluida = osStatus === "conclu\xEDda" || osStatus === "concluida" || osStatus === "completed";
  const isMissionEncerrada = osMissionStatus === "encerrada";
  return isOsConcluida && isMissionEncerrada && billingStatus !== "FATURADO" && billingStatus !== "FATURADA" && billingStatus !== "PAGO" && billingStatus !== "CANCELADO" && billingStatus !== "CANCELADA" && billingStatus !== "REJEITADA";
}
var PALETTE, desc, OS_STATUS_MAP, BILLING_STATUS_MAP;
var init_mission_status = __esm({
  "shared/constants/mission-status.ts"() {
    "use strict";
    PALETTE = {
    ...
    OS_STATUS_MAP = {
      ...
      completed: desc("Conclu\xEDda", "emerald", { appearsInMedicao: true, appearsInFaturamento: true, countsRevenue: true }),
    ...
    BILLING_STATUS_MAP = {
      ...
      ENVIADA_APROVACAO: desc("Enviada Aprova\xE7\xE3o", "blue", { appearsInMedicao: true, appearsInFaturamento: true, countsRevenue: true }),
...
function parseInvoicePeriodInfo(description, dueDateISO) {
  const desc2 = String(description || "");
  const m = desc2.match(
...
async function autoLinkOrphanBillingsForInvoice(invoice, opts = {}) {
  ...
  try {
    ...
    const desc2 = String(invoice.description || "");
    const pm = desc2.match(/(\d{2}\/\d{2}\/\d{4})\s*(?:a|até|-)\s*(\d{2}\/\d{2}\/\d{4})/i);
  ...
  if (stillOpen && customerId && targetValue > 0) {
    try {
      ...
      const candidates = (search?.data || []).filter((p) => {
        ...
        const desc2 = String(p.description || "").toLowerCase();
        return desc2.includes("pix recebido") || desc2.includes("pix recebida");
      });
      const free = [];
      for (const c of candidates) {
    ...
    try {
      ...
      if (updatedInvoice && (newStatus === "REFUNDED" || newStatus === "CANCELLED")) {
        try {
          ...
          console.error(`[asaas] Revers\xE3o autom\xE1tica falhou p/ invoice ${updatedInvoice.id}:`, e.message);
        }
      }
      await logSystemAudit({
        userId: null,
...
${osDescriptions.join("\n")}`);
      ...
      if (clientData?.billing_cycle === "quinzenal") {
        ...
        const guardSoIds = Array.from(new Set((allInPeriod || []).map((b) => b.service_order_id).filter(Boolean)));
        const guardSoById = /* @__PURE__ */ new Map();
        for (let i = 0; i < guardSoIds.length; i += 500) {
          const { data: sos } = await supabaseAdmin.from("service_orders").select("id, status, mission_status").in("id", guardSoIds.slice(i, i + 500));
          for (const so of sos || []) guardSoById.set(Number(so.id), so);
        }
        const blocking = (allInPeriod || []).filter((b) => {
  ...
  app.delete("/api/invoices/:id", requireAdminRole, async (req, res) => {
    try {
      ...
      if (linkedBillings && linkedBillings.length > 0) {
        ...
        }).catch((e) => console.error("[billing] revers\xE3o na exclus\xE3o:", e.message));
  ...
  app.post("/api/relatorio-nf/delete-row", requireAdminRole, async (req, res) => {
    try {
      ...
      }).catch((e) => console.error("[relatorio-nf] revers\xE3o na exclus\xE3o:", e.message));
...
async function sendCommand(veiID, command, mensagem) {
  ...
  try {
    const xml = `<RequestEnvioComando login="${config2.login}" senha="${config2.senha}"><comando><veiID>${veiID}</veiID><cmd>${cmdInfo.cmd}</cmd>${msgTag}</comando></RequestEnvioComando>`;
...
var init_zapi = __esm({
  "server/lib/zapi.ts"() {
    ...
    ZAPI_SEND_DELAYS_ENABLED = false;
  }
});
  ...
  for (const os of shuffle(activeOs)) {
    ...
    for (const p of shuffle(phones)) {
      try {
        ...
        if (!firstSendGlobal) await sleep(humanDelayMs(6e3, 26e3));
...
__export(cron_whatsapp_forward_exports, {
  ...
  isSuppressedPhotoCaption: () => isSuppressedPhotoCaption,
  mapsLink: () => mapsLink,
  ...
  transformClientCaptionMessage: () => transformClientCaptionMessage
});
...
function isSuppressedPhotoCaption(message) {
  return /^📷\s*foto:\s*(km\s*sa[íi]da|agente[_\s-]*equipado)\b/i.test(String(message || "").trim());
}
function transformClientCaptionMessage(message) {
  const m = String(message || "");
  const match = m.match(/^📷\s*foto:\s*km\s*chegada\b(?:\s*—\s*km\s*([\d.,]+))?/i);
  if (!match) return m;
  return match[1] ? `CHEGADA NO CLIENTE - KM INICIAL - ${match[1]}` : `CHEGADA NO CLIENTE - KM INICIAL`;
}
...
async function buildRichCaption(u, so, client, stepLabel) {
  ...
  const progressBar = (pct) => {
    ...
    return "\u{1F7E9}".repeat(filled) + "\u2B1C".repeat(5 - filled);
  };
  const L = [];
  L.push(`*MONITORAMENTO GRUPO TORRES*`);
  L.push(`*OS:* ${u.os_number || "\u2014"} | *STATUS:* ${statusLabel || "\u2014"}`);
  L.push("");
  if (dataStr || horaStr) L.push(`\u{1F5D3}\uFE0F *DATA:* ${dataStr} *HORA:* ${horaStr}`);
  L.push(`\u{1F6E1}\uFE0F *OPERA\xC7\xC3O:* CARACTERIZADA`);
  ...
  L.push("");
  if (progressoPct != null) L.push(`\u{1F4C8}*PROGRESSO DA MISS\xC3O:* ${progressBar(progressoPct)} ${progressoPct}%`);
  L.push("");
  if (addr) L.push(`\u{1F3D9}\uFE0F *LOCALIZA\xC7\xC3O:* ${addr}`);
  if (hasGeo) L.push(`\u{1F5FE} *LINK DO GOOGLE:* ${mapsLink(upLat, upLng)}`);
  L.push("");
  if (msgUpper) L.push(`\u{1F4E3} *OCORR\xCANCIA:* ${msgUpper}`);
  return L.join("\n").replace(/\n{3,}/g, "\n\n").trim();
}
  ...
  try {
    ...
    for (const u of ups) {
      ...
      if (isSuppressedPhotoCaption(msg)) {
        await markDone(u.id, "skip: foto interna (KM Sa\xEDda/Agente Equipado) n\xE3o vai pro grupo do cliente");
      ...
      if (isFinalizada) {
        const { data: priorSent, error: pfErr } = await supabaseAdmin.from("mission_updates").select("id, mission_step, message").eq("service_order_id", u.service_order_id).not("whatsapp_forwarded_at", "is", null).is("whatsapp_forward_error", null).neq("id", u.id);
        if (pfErr) {
          await releaseClaim(u.id, `db dedup-final: ${pfErr.message}`);
          console.error(`${TAG} \u27F3 id=${u.id} erro transit\xF3rio DEDUP-FINAL:`, pfErr.message);
          continue;
        }
...
function initWhatsappForwardCron() {
  ...
  setTimeout(() => processPending().catch(() => {
  }), 5e3);
}
...
async function loadActiveOs() {
  ...
  return (data || []).filter(
    (o) => !FINISHED_MISSION_STATUS2.has(String(o.mission_status || "").toLowerCase())
  );
}
...
async function buildClientSummaryByGroup(groupId) {
  ...
  const active = (activeRows || []).filter((o) => !FINISHED_MISSION_STATUS2.has(String(o.mission_status || "").toLowerCase())).sort((a, b) => String(a.mission_started_at || "").localeCompare(String(b.mission_started_at || "")));
...
async function handleGroupSummaryRequest(parsed) {
  try {
    ...
    if (Date.now() - last < SUMMARY_THROTTLE_MS) {
      console.log(`[agent-central-mention] resumo do grupo ${parsed.chatId} ignorado (throttle ${SUMMARY_THROTTLE_MS / 1e3}s)`);
      return;
    }
    const msg = await buildClientSummaryByGroup(parsed.chatId);
    if (!msg) {
      console.log(`[agent-central-mention] resumo pedido no grupo ${parsed.chatId} mas grupo n\xE3o est\xE1 vinculado a nenhum cliente \u2014 ignorando`);
      return;
    }
...
async function buildUpdateAck(text, senderName) {
  ...
  try {
    ...
    const response = await openai.chat.completions.create({
      ...
      messages: [
        {
          role: "system",
          content: `Voc\xEA \xE9 a "Central Torres", atendente HUMANA de uma empresa de escolta/seguran\xE7a, num grupo de WhatsApp de um CLIENTE. O cliente acabou de pedir uma ATUALIZA\xC7\xC3O de uma miss\xE3o. Responda com UMA frase curta, natural e cordial confirmando que VAI VERIFICAR/COBRAR a equipe e j\xE1 retorna. VARIE SEMPRE as palavras (nunca soe como template \u2014 isso causa bloqueio do WhatsApp). Portugu\xEAs brasileiro, no m\xE1ximo 1 emoji. NUNCA invente dado operacional (hor\xE1rios, locais, KMs, status, previs\xF5es) e NUNCA fale de valores/financeiro. N\xC3O prometa hor\xE1rio espec\xEDfico. Responda S\xD3 com a frase, sem aspas.`
        },
        {
          role: "user",
...
async function handleTaggedUpdateAck(parsed) {
  try {
    ...
    if (Date.now() - last < TAGGED_ACK_THROTTLE_MS) {
      console.log(`[agent-central-mention] aviso de cobran\xE7a no grupo ${parsed.chatId} ignorado (throttle ${TAGGED_ACK_THROTTLE_MS / 1e3}s)`);
      return;
    }
    ...
    console.log(`[agent-central-mention] aviso de cobran\xE7a enviado ao grupo ${parsed.chatId} (digitando ~15s)`);
  } catch (e) {
...
async function handleGroupUpdateRequest(parsed, rawBody) {
  try {
    ...
    console.log(`[agent-central-mention] OS ${os.os_number || os.id} no grupo ${parsed.chatId}: avisou no grupo + 1\xBA agente cobrado por DM; escalonamento p/ 2\xBA armado em ${ESCALATE_AFTER_MIN}min se n\xE3o houver resposta`);
  } catch (e) {
...
async function fulfillGroupRequests(params) {
  try {
    ...
    const { data: open, error: claimErr } = await supabaseAdmin.from("agent_central_group_requests").update({ fulfilled_at: claimedAt }).eq("service_order_id", params.serviceOrderId).is("fulfilled_at", null).select("id, group_id, requester_name");
    if (claimErr) {
    ...
    for (const r of open) {
      ...
      if (!byGroup.has(g)) byGroup.set(g, { ids: [], names: /* @__PURE__ */ new Set() });
    ...
    if (params.hadPhoto && isForwardableStep(params.missionStep) && !isSuppressedPhotoCaption(params.message)) {
      console.log(`[agent-central-mention] OS ${params.osNumber || params.serviceOrderId}: card com foto ser\xE1 enviado pelo cron de encaminhamento; fulfill apenas resolveu ${open.length} pedido(s).`);
      return;
    }
...
_Agente: ${firstName(params.employeeName)}_` : "",
      ...
      if (r.ok) {
        ...
        console.warn(`[agent-central-mention] envio ao grupo ${groupId} bloqueado pela trava do bot \u2014 pedido descartado (n\xE3o ser\xE1 reenviado ap\xF3s reconex\xE3o)`);
      } else {
...
var init_agent_central_mention = __esm({
  "server/lib/agent-central-mention.ts"() {
    ...
    ACK_FALLBACKS = [
      "Opa! Deixa comigo, j\xE1 vou verificar com a equipe \u{1F44D}",
      "Pode deixar! T\xF4 cobrando o time agora e j\xE1 te retorno.",
      "Show, vou acionar a equipe agora mesmo e trago a atualiza\xE7\xE3o!",
      "Opa, beleza! J\xE1 estou cobrando a equipe pra te atualizar.",
      "Deixa comigo, \xE9 pra isso que t\xF4 aqui! J\xE1 verifico e retorno.",
      "Certo! Vou cobrar a equipe agora e j\xE1 te dou um retorno.",
      "Entendido! T\xF4 verificando com o time e j\xE1 volto com a posi\xE7\xE3o.",
      "Opa! J\xE1 vou atr\xE1s da equipe pra te trazer a atualiza\xE7\xE3o \u{1F44D}"
    ];
  }
});
...
__export(cron_exports, {
  ...
  sendVencimentosDoDiaEmail: () => sendVencimentosDoDiaEmail2
});
...
function initCronJobs() {
  ...
  cron2.schedule("0 0,12 * * *", async () => {
    if (controlIdRunning) return;
  ...
  async function runInterReconcile2(diasJanela, contexto) {
    ...
    for (const tx of transacoes) {
      ...
      if (candidateInvoices && candidateInvoices.length > 1) {
        ...
        log(`CRON Inter-Reconcile[${contexto}]: AMBIGUO \u2014 ${candidateInvoices.length} invoices com valor R$ ${tx.valor} em ${tx.dataEntrada}. Concilia\xE7\xE3o manual necess\xE1ria.`, "cron");
      }
    ...
    if (novosLancamentos > 0) {
      log(`CRON Inter-Reconcile[${contexto}]: ${novosLancamentos} lan\xE7amento(s), ${conciliados} invoice(s) conciliada(s)`, "cron");
    }
  }
  ...
  cron2.schedule("0 6 2-5 * *", async () => {
    ...
    try {
      ...
      if (r) log(`CRON Folha-Snapshot[catch-up]: m\xEAs=${r.mes} ativos=${r.ativos} salvos=${r.saved} pulados=${r.skipped}`, "cron");
    } catch (e) {
  ...
  cron2.schedule("*/5 * * * *", async () => {
    ...
    try {
      await runInterReconcile2(2, "5min/2d");
    } catch (e) {
  ...
  cron2.schedule("0 4 * * *", async () => {
    ...
    try {
      ...
      await runInterReconcile2(30, "backfill/30d");
    } catch (e) {
  ...
  cron2.schedule("0 6 * * *", async () => {
    try {
      ...
      if (r.paresLongosDetectados > 0 || r.linhasCriadas > 0) {
        log(`CRON Di\xE1rias>16h: ${r.quinzena} (${r.quinzenaInicio}\u2192${r.quinzenaFim}) pares=${r.paresLongosDetectados} agentes=${r.agentes.length} criadas=${r.linhasCriadas} removidas=${r.linhasRemovidas}`, "cron");
      }
    } catch (e) {
      log(`CRON Di\xE1rias>16h: Erro: ${e.message}`, "cron");
    }
  }, { timezone: "America/Sao_Paulo" });
  cron2.schedule("10 3 * * *", async () => {
    try {
  ...
  cron2.schedule("30 9 * * 1-5", () => {
    log("CRON Rod\xEDzio: Disparando alerta das 06:30 BRT", "cron");
    sendRodizioAlerts().catch((err) => log(`CRON Rod\xEDzio: Erro: ${err.message}`, "cron"));
  });
  cron2.schedule("30 19 * * 1-5", () => {
    log("CRON Rod\xEDzio: Disparando alerta das 16:30 BRT", "cron");
    sendRodizioAlerts().catch((err) => log(`CRON Rod\xEDzio: Erro: ${err.message}`, "cron"));
  });
  cron2.schedule("*/10 * * * *", async () => {
  ...
  cron2.schedule("59 2 * * *", async () => {
    ...
    try {
      ...
      const CCT = { salarioBase: 2432.5, periculosidadePct: 30, valeRefeicaoDia: 40, cestaBasica: 208.45, diasUteisMes: 22, horaExtraValor: 22.99 };
      ...
      const activeEmployees = allEmployees.filter((e) => e.status === "ativo" && (e.role?.toLowerCase().includes("vigilante") || e.role?.toLowerCase().includes("escolta")));
      let created = 0;
      let skipped = 0;
      for (const emp of activeEmployees) {
      ...
      log(`CRON Provis\xE3o: ${brDate} \u2014 ${created} provis\xE3o(\xF5es) criada(s), ${skipped} ignorada(s) (${activeEmployees.length} agentes ativos)`, "cron");
    } catch (err) {
      log(`CRON Provis\xE3o: Erro geral: ${err.message}`, "cron");
    }
  });
  cron2.schedule("0 11 * * *", async () => {
    try {
    ...
    try {
      ...
      for (const v of vehicles) {
        ...
        if (nextMaint?.length && nextMaint[0].next_maintenance_km) {
          ...
          if (kmUntil <= 0) {
            alerts.push(`\u{1F534} ${label}: Manuten\xE7\xE3o "${nextMaint[0].type}" VENCIDA (KM ${nextMaint[0].next_maintenance_km.toLocaleString()} ultrapassado)`);
          } else if (kmUntil <= 1e3) {
...
${alerts.join("\n")}`
  ...
  cron2.schedule("0 6,9,12,15,18 * * 1-5", async () => {
    try {
  ...
  cron2.schedule("0 7 * * *", async () => {
    try {
      ...
      log(`CRON DocCompliance: ${r.message} \u2014 ${r.employees} funcion\xE1rio(s), ${r.totalMissing} faltante(s), ${r.totalExpired} vencido(s)`, "cron");
    } catch (e) {
  ...
  cron2.schedule("*/5 * * * *", async () => {
    ...
    try {
      ...
      if (r.reminded > 0 || r.skipped_nophone > 0) {
        log(`CRON AgenteCentral: ${r.scanned} OSs ativas, ${r.reminded} cobran\xE7as enviadas, ${r.skipped_nophone} sem telefone`, "cron");
      }
    } catch (e) {
  ...
  cron2.schedule("* * * * *", async () => {
    ...
    try {
      ...
      if (r.escalated > 0 || r.fulfilled > 0 || r.no_second > 0) {
        log(`CRON AgenteCentral-Escalonamento: ${r.escalated} 2\xBA agente(s) cobrado(s), ${r.fulfilled} resolvido(s) (1\xBA respondeu), ${r.no_second} sem 2\xBA agente`, "cron");
      }
    } catch (e) {
  ...
  log("CRON: Tarefas agendadas - Frota (di\xE1rio 02:00) | RH (trimestral dia 1 \xE0s 03:00) | Rod\xEDzio (seg-sex 06:30 e 16:30 BRT) | Billing (a cada 30min) | BillingAlerts (di\xE1rio 03:00 BRT) | Provis\xE3o Sal\xE1rio (di\xE1rio 23:59 BRT) | JornadaAlerta (di\xE1rio 08:00 BRT) | AceiteExpirado (a cada 30min) | AlertaFrota (di\xE1rio 07:00) | AlertaDocRH (di\xE1rio 08:00) | DocCompliance (di\xE1rio 07:00 BRT) | ResumoFinanceiro (seg-sex 06h/09h/12h/15h/18h BRT \u2014 diretoria) | ControlID (00:00 e 12:00 BRT) | AgenteCentral (proativo DESATIVADO \u2014 s\xF3 cobra quando pedem no grupo) | AgenteCentral-Escalonamento (a cada 1min \u2014 cobra 2\xBA agente se 1\xBA n\xE3o responder) | Cobran\xE7aVencidos (DESATIVADO \u2014 s\xF3 envio manual)", "cron");
}
...
async function runVencimentosCron() {
  try {
    log("CRON Vencimentos: disparando e-mail di\xE1rio", "cron");
    await sendVencimentosDoDiaEmail2();
  } catch (err) {
    log(`CRON Vencimentos: erro: ${err.message}`, "cron");
  }
}
...
async function runCronBucket(bucket) {
  switch (bucket) {
    case "minute": {
      const { processPendingForwards } = await Promise.resolve().then(() => (init_cron_whatsapp_forward(), cron_whatsapp_forward_exports));
