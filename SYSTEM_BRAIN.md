...
| **Jurídico / Cobrança Judicial** | `admin/cobranca-judicial.tsx` (grupo Financeiro no menu) | `routes/cobranca-judicial.ts` (`/api/invoices/:id/dossie-juridico[/pdf\|/zip]`, `/api/invoices/:id/cobranca-judicial`, `/api/cobranca-judicial[/:id]`, `/api/juridico/dossie/:token` PÚBLICO) | `buildDossie` (agrega cadastro→contrato→execução→NF→boleto→pagamento), `buildDossiePdf`, `streamDossieZip` (anti-SSRF `isSafeRemoteUrl`); guard de bloqueio de OS em `routes/service-orders.ts` POST | §8 (NUNCA toca invoices.status — estado jurídico só na tabela `cobranca_judicial`) |
